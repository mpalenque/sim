...
export function createGUI({ viewer, hooks }) {
  ...
  const global = gui.addFolder('Iluminación global');
  ...
  const cascades = gui.addFolder('Cascadas');
  ...
  const voxels = gui.addFolder('Voxelización');
  ...
  const screen = gui.addFolder('PANTALLA');
  ...
  const mirror = gui.addFolder('Espejo (base.001)');
  ...
  const reflections = gui.addFolder('Reflejos');
  ...
  const direct = gui.addFolder('Luces directas');
  ...
  const output = gui.addFolder('Calidad / salida');
